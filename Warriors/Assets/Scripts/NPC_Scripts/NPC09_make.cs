...
public class NPC09_make : MonoBehaviour, IAnimEventListener
{
    ...
    void Awake()
    {
        ...
        NPC09_struct.gameobject = GameObject.Find("_NPC09_gameobj");
        NPC09_struct.npc_enable = GameObject.Find("_npc9_enable");                                             // NPC02하위 요소들을 enable할지 말지 결정할 val.
        ...
        NPC09_struct.add_speed_label.GetComponent<UILabel>().text = "+0%";

        NPC09_struct.skill_label = GameObject.Find("_npc09_skill_label");
    ...
    void Start()
    {
        ...
        else
        {
            ...
            NPC09_struct.npc_enable.SetActive(false);   // npc 하위 변수들 비활성화. ( 해당 npc가 enable이 아닌데도 버튼이 눌려지는 현상을 해결하기 위함. )
        ...
        {
            NPC09_struct.npc_enable.SetActive(true);
    ...
    public void init () {
        ...
        if (npc09_char.weapon_enable == 1)
        {
            ...
            GameData_weapon.equip_the_weapon(weapon_index, popup_window_button_mgr.NPC_INDEX.NPC09);
        ...
        if (npc09_char.armor_enable == 1)
        {
            ...
            GameData_weapon.get_armor_skill_func(someValue, 8);
        ...
        if (npc09_char.wing_enable == 1)
        {
            ...
            print("npc9이 현재 장착하고 있는 wing index : " + equip_wing_index);
            GameData_weapon.get_wing_skill_func(equip_wing_index, popup_window_button_mgr.NPC_INDEX.NPC09);
    ...
    public void levelup_npc09_data_struct(int Level)
    {
        ...
        // 캐릭터 공격력도 누적.
        NPC09_struct.damage = 0;

        for (int i = 1; i < Level + 1; i++)
        {
            NPC09_struct.damage = NPC09_struct.damage + (ulong)(720 * Mathf.Pow(1.275f, i) + 10);
        }

        // =POWER(1.325,A2)*10000
        NPC09_struct.upgrade_cost = (ulong)Mathf.Round(Mathf.Pow(1.325f, Level) * 100000000);


        // NPC03 레벨이 20 이상이면 NPC04 캐릭터 구입할 수 있음.
        if (NPC09_struct.Level == 2)
        {
            // NPC10 Level up 캐릭터 창 Enable 시켜줌. ( 단, 아직은 NPC04 캐릭터는 화면에 안보여짐. )
            NPC10_make.NPC10_struct.unlock_sp.SetActive(false);
            NPC10_make.NPC10_struct.npc_enable.SetActive(true);
    ...
    public void update_npc09_data_label()
    {
        ...
        NPC09_struct.lvup_cost_label.GetComponent<UILabel>().text = GameData.int_to_label_format_won(NPC09_struct.upgrade_cost);
    ...
    public void change_weapon(int enable, int weapon_index, string weapon_name)
    {
        ...
        npc09_char.weapon_enable = enable;
        ...
        PlayerPrefs.SetString("npc9_weapon_part", npc09_char.weapon_part);
        PlayerPrefs.SetInt("npc9_weapon_index", npc09_char.weapon_index);
        PlayerPrefs.Save();
    ...
    public void change_clothes(int enable, int index, int color, string clothes_type)
    {
        ...
        // Boss Scene Load시 사용할 character image;
        npc09_char.armor_enable = enable;
    ...
    public void change_attack_speed(float speed, float anim_speed)
    {
        ...
        saved_attack_speed = NPC09_struct.attack_speed;
        
        NPC09_struct.attack_speed = 1 * speed;
        anim.speed = anim_speed;

    }
    public void reset_attack_speed()
    {
        NPC09_struct.attack_speed = 1;
    }
     public void ads_reset_attack_speed()
    {
        ...
        NPC09_struct.attack_speed = saved_attack_speed;
    }
}
