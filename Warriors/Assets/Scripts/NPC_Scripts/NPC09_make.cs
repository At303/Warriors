...
public class NPC09_make : MonoBehaviour, IAnimEventListener
{
    ...
    public void levelup_npc09_data_struct()
    {
        if (NPC09_struct.Level == 1)
        {
            // 맨 처음 NPC03 레벨업 버튼 클릭시에는 해당 캐릭터가 disable되어 있으므로 Enable시켜줌.
            NPC09_struct.gameobject.SetActive(true);
            init();
        }

        // NPC03 데이터 초기화 및 레벨업시 적용되는 공식.
        NPC09_struct.Level = NPC09_struct.Level + 1;
