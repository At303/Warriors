...
public class NPC10_make : MonoBehaviour, IAnimEventListener
{
    ...
    public static float npc10_saved_attack_speed = 0f;
    ...
    void Awake()
    {
        ...
        NPC10_struct.npc_enable = GameObject.Find("_npc10_enable");                                             // NPC02하위 요소들을 enable할지 말지 결정할 val.
        ...
        NPC10_struct.skill_label = GameObject.Find("_npc10_skill_label");
    ...
    void Start()
    {
        ...
        if (check_npc_enable == 1)
        {
            ...
            NPC10_struct.gameobject.SetActive(true);                 // npc 캐릭터 활성화.
        ...
        else
        {
            ...
            NPC10_struct.npc_enable.SetActive(false);   // npc 하위 변수들 비활성화. ( 해당 npc가 enable이 아닌데도 버튼이 눌려지는 현상을 해결하기 위함. )
        ...
        int check_npc_level = PlayerPrefs.GetInt("npc9_level", 0);
        if (check_npc_level > 2)
        {
            NPC10_struct.npc_enable.SetActive(true);
            NPC10_struct.unlock_sp.SetActive(false);                  // npc 아이템 캐릭터창 unlock 풀어주기
        }
    }
    ...
    public void init () {
        ...
        if (npc10_char.armor_enable == 1)
        {
            ...
            GameData_weapon.set_data_for_equip_armor(someValue, 9);

            // Change the NPC05 Clothes icon Sprite.
            NPC10_struct.clothes_sp.atlas = Resources.Load<UIAtlas>("BackgroundAtlas");
            NPC10_struct.clothes_sp.spriteName = character.Info.armor_part + character.Info.armor_index.ToString() + character.Info.armor_color.ToString();
        ...
        if (npc10_char.wing_enable == 1)
        {
            ...
            GameData_weapon.set_data_for_equip_wing(character.Info.wing_part, character.Info.wing_index, popup_window_button_mgr.NPC_INDEX.NPC10);
    ...
    public void levelup_npc10_data_struct(int Level)
    {
        ...
        // 캐릭터 공격력도 누적.
        NPC10_struct.damage = 0;

        for (int i = 1; i < Level + 1; i++)
        {
            NPC10_struct.damage = NPC10_struct.damage + (ulong)(800 * Mathf.Pow(1.275f, i) + 10);
        }

        // =POWER(1.325,A2)*10000
        NPC10_struct.upgrade_cost = (ulong)Mathf.Round(Mathf.Pow(1.325f, Level) * 500000000);

        // NPC04 레벨이 20 이상이면 NPC04 캐릭터 구입할 수 있음.
        if (NPC10_struct.Level == 3)
        {
            // NPC11 Level up 캐릭터 창 Enable 시켜줌. ( 단, 아직은 NPC11 캐릭터는 화면에 안보여짐. )
            NPC11_make.NPC11_struct.unlock_sp.SetActive(false);
            NPC11_make.NPC11_struct.npc_enable.SetActive(true);
    ...
    public void update_npc10_data_label()
    {
        ...
        NPC10_struct.lvup_cost_label.GetComponent<UILabel>().text = GameData.int_to_label_format_won(NPC10_struct.upgrade_cost);
    ...
    {
        npc10_saved_attack_speed = NPC10_struct.attack_speed;
        NPC10_struct.attack_speed = NPC10_struct.attack_speed * 0.5f;
    }
    public void reset_attack_speed()
    {
        NPC10_struct.attack_speed = npc10_saved_attack_speed;
    }
}
