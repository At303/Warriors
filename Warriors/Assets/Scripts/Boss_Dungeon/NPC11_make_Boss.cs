...
public class NPC11_make_Boss : MonoBehaviour, IAnimEventListener
{
    // NPC struct
    public struct NPC11_Boss_struct
    {
        // NPC11 Data. 
    ...
    void Start ()
    {
        ...
        if (check_npc_enable == 1)
        {
            ...
            GameObject.Find("_NPC11_gameobj_intheboss").SetActive(true);                 // npc 캐릭터 활성화.
        ...
        {
            GameObject.Find("_NPC11_gameobj_intheboss").SetActive(false);                 // npc11 캐릭터 비활성화.
        }
    }
    ...
    public void init()
    {
        ...
        if (PlayerPrefs.GetInt("npc11_weapon_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc11_armor_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc11_wing_enable", 0) == 1)
        {
            ...
            Animator anim = GameObject.Find("Impl11 ").GetComponent<Animator>();
        ...
        NPC11_Boss_struct.damage = NPC11_make.NPC11_struct.damage;
        NPC11_Boss_struct.add_damage = NPC11_make.NPC11_struct.add_damage;

        // 무기에 따라서 해당 값을 변경해주면될듯.
        NPC11_Boss_struct.attack_speed = NPC11_make.NPC11_struct.attack_speed;


        character.InitWithoutTextureBaking();

        // Add Attack event clip interface. ( NPC11이 공격 애니메이션 시 사용할 함수를 추가. )
    ...
    {
        yield return new WaitForSeconds(NPC11_Boss_struct.attack_speed);             // 모든 NPC 공격 default속도는 1. attack. 무한반복.
    ...
    public void OnAnimation_Hitting(int _index)
    {
        ...
        if (GM_Boss.boss_hp > 0)
        {
            ...
            // Damage HUDText.
            string get_coin_str = "-" + (NPC11_Boss_struct.damage + NPC11_Boss_struct.add_damage).ToString();
            GM_Boss.slash_HUD.GetComponent<HUDText>().Add(get_coin_str, Color.red, 0.5f);

            // Chest box HP modify.
            GM_Boss.boss_hp = GM_Boss.boss_hp - (NPC11_Boss_struct.damage + NPC11_Boss_struct.add_damage);
    ...
    /// NPC11 처음 init시 현재저장되어 있는 NPC Level에 맞게 값을 설정해줄 Function.
    public void levelup_npc11_data_struct(int Level)
    {

        // NPC11 데이터 초기화 및 레벨업시 적용되는 공식.
        NPC11_Boss_struct.Level = Level;
        NPC11_Boss_struct.damage = (ulong)(NPC11_Boss_struct.Level * 2) + 7;
        NPC11_Boss_struct.attack_speed = NPC11_make.NPC11_struct.attack_speed;
    }
