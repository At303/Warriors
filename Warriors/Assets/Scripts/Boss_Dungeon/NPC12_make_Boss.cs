...
{
    // NPC struct
    public struct NPC12_Boss_struct
    {
        // NPC12 Data. 
    ...
    void Start ()
    {
        ...
        if (check_npc_enable == 1)
        {
            ...
            GameObject.Find("_NPC12_gameobj_intheboss").SetActive(true);                 // npc 캐릭터 활성화.
        ...
        {
            GameObject.Find("_NPC12_gameobj_intheboss").SetActive(false);                 // npc12 캐릭터 비활성화.
        }
    }
    ...
    public void init()
    {
        ...
        if (PlayerPrefs.GetInt("npc12_weapon_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc12_armor_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc12_wing_enable", 0) == 1)
        {
            ...
            Animator anim = GameObject.Find("Impl12").GetComponent<Animator>();
        ...
        NPC12_Boss_struct.damage = NPC12_make.NPC12_struct.damage;
        NPC12_Boss_struct.add_damage = NPC12_make.NPC12_struct.add_damage;

        // 무기에 따라서 해당 값을 변경해주면될듯.
        NPC12_Boss_struct.attack_speed = NPC12_make.NPC12_struct.attack_speed;
        
        character.InitWithoutTextureBaking();

        // Add Attack event clip interface. ( NPC12이 공격 애니메이션 시 사용할 함수를 추가. )
    ...
    {
        yield return new WaitForSeconds(NPC12_Boss_struct.attack_speed);             // 모든 NPC 공격 default속도는 1. attack. 무한반복.
    ...
    public void OnAnimation_Hitting(int _index)
    {
        ...
        if (GM_Boss.boss_hp > 0)
        {
            ...
            // Damage HUDText.
            string get_coin_str = "-" + (NPC12_Boss_struct.damage + NPC12_Boss_struct.add_damage).ToString();
            GM_Boss.slash_HUD.GetComponent<HUDText>().Add(get_coin_str, Color.red, 0.5f);

            // Chest box HP modify.
            GM_Boss.boss_hp = GM_Boss.boss_hp - (NPC12_Boss_struct.damage + NPC12_Boss_struct.add_damage);
    ...
    /// NPC12 처음 init시 현재저장되어 있는 NPC Level에 맞게 값을 설정해줄 Function.
    public void levelup_npc12_data_struct(int Level)
    {

        // NPC12 데이터 초기화 및 레벨업시 적용되는 공식.
        NPC12_Boss_struct.Level = Level;
        NPC12_Boss_struct.damage = (ulong)(NPC12_Boss_struct.Level * 2) + 7;
        NPC12_Boss_struct.attack_speed = NPC12_make.NPC12_struct.attack_speed;
    }
