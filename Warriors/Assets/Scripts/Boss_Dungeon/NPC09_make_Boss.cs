...
public class NPC09_make_Boss : MonoBehaviour, IAnimEventListener
{
    // NPC struct
    public struct NPC09_Boss_struct
    {
        // NPC09 Data. 
    ...
    void Start ()
    {
        ...
        if (check_npc_enable == 1)
        {
            ...
            GameObject.Find("_NPC09_gameobj_intheboss").SetActive(true);                 // npc 캐릭터 활성화.
        ...
        {
            GameObject.Find("_NPC09_gameobj_intheboss").SetActive(false);                 // npc9 캐릭터 비활성화.
        }
    }
    ...
    public void init()
    {
        ...
        if (PlayerPrefs.GetInt("npc9_weapon_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc9_armor_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc9_wing_enable", 0) == 1)
        {
            ...
            Animator anim = GameObject.Find("Impl9").GetComponent<Animator>();
        ...
        NPC09_Boss_struct.damage = NPC09_make.NPC09_struct.damage;
        NPC09_Boss_struct.add_damage = NPC09_make.NPC09_struct.add_damage;

        // 무기에 따라서 해당 값을 변경해주면될듯.
        NPC09_Boss_struct.attack_speed = NPC09_make.NPC09_struct.attack_speed;
        
        character.InitWithoutTextureBaking();

        // Add Attack event clip interface. ( NPC09이 공격 애니메이션 시 사용할 함수를 추가. )
    ...
    {
        yield return new WaitForSeconds(NPC09_Boss_struct.attack_speed);             // 모든 NPC 공격 default속도는 1. attack. 무한반복.
    ...
    public void OnAnimation_Hitting(int _index)
    {
        ...
        if (GM_Boss.boss_hp > 0)
        {
            ...
            // Damage HUDText.
            string get_coin_str = "-" + (NPC09_Boss_struct.damage + NPC09_Boss_struct.add_damage).ToString();
            GM_Boss.slash_HUD.GetComponent<HUDText>().Add(get_coin_str, Color.red, 0.5f);

            // Chest box HP modify.
            GM_Boss.boss_hp = GM_Boss.boss_hp - (NPC09_Boss_struct.damage + NPC09_Boss_struct.add_damage);
    ...
    /// NPC09 처음 init시 현재저장되어 있는 NPC Level에 맞게 값을 설정해줄 Function.
    public void levelup_npc09_data_struct(int Level)
    {

        // NPC09 데이터 초기화 및 레벨업시 적용되는 공식.
        NPC09_Boss_struct.Level = Level;
        NPC09_Boss_struct.damage = (ulong)(NPC09_Boss_struct.Level * 2) + 7;
        NPC09_Boss_struct.attack_speed = NPC09_make.NPC09_struct.attack_speed;
    }
}
