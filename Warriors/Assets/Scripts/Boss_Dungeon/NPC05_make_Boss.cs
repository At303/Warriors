...
public class NPC05_make_Boss : MonoBehaviour, IAnimEventListener
{
    // NPC struct
    public struct NPC05_Boss_struct
    {
        // NPC05 Data. 
    ...
    void Start ()
    {
        ...
        if (check_npc_enable == 1)
        {
            ...
            GameObject.Find("_NPC05_gameobj_intheboss").SetActive(true);                 // npc 캐릭터 활성화.
        ...
        {
            GameObject.Find("_NPC05_gameobj_intheboss").SetActive(false);                 // npc5 캐릭터 비활성화.
        }
    }
    ...
    public void init()
    {
        ...
        if (PlayerPrefs.GetInt("npc5_weapon_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc5_armor_enable", 0) == 1)
        {
        ...
        if (PlayerPrefs.GetInt("npc5_wing_enable", 0) == 1)
        {
            ...
            Animator anim = GameObject.Find("Impl5").GetComponent<Animator>();
        ...
        NPC05_Boss_struct.damage = NPC05_make.NPC05_struct.damage;
        NPC05_Boss_struct.add_damage = NPC05_make.NPC05_struct.add_damage;
        ...
        // Add Attack event clip interface. ( NPC05이 공격 애니메이션 시 사용할 함수를 추가. )
    ...
    {
        yield return new WaitForSeconds(NPC05_Boss_struct.attack_speed);             // 모든 NPC 공격 default속도는 1. attack. 무한반복.
    ...
    public void OnAnimation_Hitting(int _index)
    {
        ...
        if (GM_Boss.boss_hp > 0)
        {
            ...
            // Damage HUDText.
            string get_coin_str = "-" + (NPC05_Boss_struct.damage + NPC05_Boss_struct.add_damage).ToString();
            GM_Boss.slash_HUD.GetComponent<HUDText>().Add(get_coin_str, Color.red, 0.5f);

            // Chest box HP modify.
            GM_Boss.boss_hp = GM_Boss.boss_hp - (NPC05_Boss_struct.damage + NPC05_Boss_struct.add_damage);
    ...
    /// NPC05 처음 init시 현재저장되어 있는 NPC Level에 맞게 값을 설정해줄 Function.
